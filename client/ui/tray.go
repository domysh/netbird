...
const (
	...
	// such state — it reports Connected right up until the tunnel is down — so
	// the tray supplies it for as long as a Down is in flight.
...
type Tray struct {
	...
	// the row saying Connected — the daemon keeps reporting that until the
	// tunnel is actually down — and the switch flicks back on under the user,
	// or shows itself connected again when the menu is reopened.
	disconnecting bool
	...
	// menuDismissedAt is when a connection change last took the menu away from
	// the user, so it can be handed back once the connection settles. Nanosecond
	// clock, zero when there is nothing to hand back.
	menuDismissedAt atomic.Int64
	// reopening guards against two reopen attempts racing each other.
...
func (t *Tray) relayoutMenu() {
	...
	if trayMenuIsOpen() {
		...
		// macOS is told when the menu closes; Windows has to watch for it.
...
// back: a popup that reappears long after the user stopped caring is worse than
// one that never does.
const menuReopenWindow = 30 * time.Second

// handleConnectionChange deals with the menu when the connection moves.
//
// Where an open menu cannot be trusted to redraw itself (Windows, see
// dismissTrayMenu) it is dismissed, and the fact is remembered: once the
// connection settles the menu is rebuilt and offered back, which is what the
// user was in the middle of reading. A transient state — connecting, going down
// — only takes the menu away; there is nothing worth reopening onto yet.
func (t *Tray) handleConnectionChange(settled bool) {
	if trayMenuIsOpen() {
		dismissTrayMenu()
		t.menuDismissedAt.Store(time.Now().UnixNano())
		// The rebuild the dismissal frees up still needs running.
		watchMenuClose(t.onTrayMenuClosed)
	}
...
// reopenTrayMenu waits for the dismissed menu to be gone, rebuilds it so the
// rows are the new ones, and shows it again. OpenMenu blocks for as long as the
// user keeps it up, hence the goroutine.
...
// onTrayMenuClosed catches the menu up with everything that was held back
// while it was open. Called from the AppKit menu-tracking observer, on a
// goroutine of its own.
//
// Two things waited: a rebuild of the rows, and the Wails-side copy of the
// state, which the AppKit painter wrote past. The relayout repaints on its own;
// without one, a refresh puts Wails back in sync so the next open starts from
// the truth.
//
// The loop covers a rebuild requested between relayoutMenu's open check and the
// close, which would otherwise sit unapplied until the next one; it stops as
// soon as the user reopens the menu, leaving the flag set for the next close.
func (t *Tray) onTrayMenuClosed() {
	// The icon was painted into the menu bar directly while the menu was up;
	// this pushes the same picture through Wails so its cached copy agrees.
	t.applyIcon()
	relaid := false
...
// refreshMenuState repaints the menu rows that carry daemon state — status,
// the connection switch, the session countdown, the enabled gates.
//
// Where the platform paints its menu natively (macOS, see nativeMenu) the live
// items are written where they stand, so a status
// flip lands in a menu the user is already looking at: the macOS status item
// shows the NSMenu handed to AppKit when the menu was clicked open, and
// SetMenu only swaps the menu shown on the *next* click. That is why a
// connect/disconnect used to need the menu closed and reopened to show its
// result.
//
// Everywhere else this falls back to a full relayout, which is what the
// dbusmenu hosts need (see relayoutMenu).
...
// menuPainter writes state onto the menu. Which implementation is in use is not
// a platform choice but a moment-by-moment one: Wails owns the rows whenever
// the menu is closed — and everywhere off macOS — while the AppKit painter
// takes over for as long as the user holds the menu open, because Wails'
// setters cannot land then (see tray_native_menu_darwin.h).
...
func (t *Tray) buildMenu() *application.Menu {
	...
	// in its label (see tray_native_menu_darwin.h); the checkbox below is what
...
func (t *Tray) handleToggleConnection() {
	t.statusMu.Lock()
	status, connected := t.effectiveStatusLocked()
	t.statusMu.Unlock()
...
// connectionRowLabel is what the connection row says, and it depends on whether
// the row carries a switch.
//
// Next to a switch it is the status itself — connected, connecting,
// disconnected — so the row states the connection and offers the control on one
// line, and there is no status row above to repeat it.
//
// Without a switch the status row above does the stating, so this row does the
// acting and says what the click will do. It used to be a checkbox labelled
// with the state, which left the menu answering the same question twice: a
// green dot reading "Connected" over a ticked box reading "Connected", and
// neither of them telling the user what clicking would achieve.
...
// effectiveStatusLocked is the status the tray shows, which is the daemon's own
// except while a Down is in flight: the daemon has no disconnecting state, so
// the tray supplies one rather than claim a connection that is going away.
// Callers must hold statusMu.
...
func (t *Tray) handleDisconnect() {
	...
	// raised for exactly that stretch and the row says so while it lasts.
	t.statusMu.Lock()
	...
	go func() {
		...
		// Repaint either way: on failure to roll the row back to what the
		// daemon reports, on success because the Idle push may already have
		// been painted while the flag was still up.
		t.refreshMenuState()
		t.applyIcon()
	}()
}
