...
// status row that keeps the words and the dot (see tray_native_menu_darwin.go
...
//
// A painted row reaches the live HMENU, but Win32 repaints the popup when it
// sees fit, so a connection change dismisses the menu instead of leaving a
// stale row on screen. Posted rather than called: EndMenu ends only the calling
// thread's menu, and this runs on a goroutine.
